...
nn_test = nn.NeuralNetwork(testing_dict, lr = 0.6, seed = 10, batch_size = 1, epochs = 2, loss_function = 'mse')
...
def test_forward():
    ...
    Z2_expected = np.array([[456., 457., 458., 459.]])
    A2_expected = np.array([[1., 1., 1., 1.]])

    assert np.array_equal(Z2_expected, cache['Z2']) == True
    assert np.array_equal(A2_expected, cache['A2']) == True
...
def test_single_forward():
    ...
    assert np.array_equal(cache['Z1'], Z_curr) == True
    assert np.array_equal(cache['A1'], A_curr) == True
...
def test_single_backprop():
    ...
    assert np.array_equal(exp_dA_prev, dA_prev) == True
    assert np.array_equal(exp_dW_curr, dW_curr) == True
...
def test_predict():
    ...
    assert np.array_equal(predict_test, exp_predict_test) == True
...
def test_one_hot_encode():
    ...
    encodings = preprocess.one_hot_encode_seqs(test_list)
...
def test_sample_seqs():
    ...
    positive_dataset = [i for i in test_dataset if i[0]=="Green"]
    negative_dataset = [i for i in test_dataset if i[0]=="Red"]

    # create an imbalance
    positive_dataset = list(repeat(positive_dataset, 3))
    positive_dataset = [item for sublist in positive_dataset for item in sublist]
    negative_dataset = list(repeat(negative_dataset, 6))
    negative_dataset = [item for sublist in negative_dataset for item in sublist]
    ...
    # testing sample_seqs
    sampled_seqs, sampled_labels = preprocess.sample_seqs(test_seqs, test_labels, 0.50)
